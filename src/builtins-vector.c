...
BUILTIN(u8vector?, u8vector_p, 1, 33)
{
	if (IN_RAM(reg1)) {
...
BUILTIN(#%make-u8vector, make_u8vector, 1, 34)
{
	reg1 = new_vector(decode_int(reg1));
}

BUILTIN(u8vector-ref, u8vector_ref, 2, 35)
{
	a2 = decode_int(reg2);
...
BUILTIN_UNSPEC(u8vector-set!, u8vector_set, 3, 36)
...
BUILTIN(u8vector-length, u8vector_length, 1, 37)
{
	if (IN_RAM(reg1)) {
