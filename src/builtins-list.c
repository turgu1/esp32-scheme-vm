...
BUILTIN_UNSPEC(set-car!, set_car_bang, 2, 10)
{
	if (IN_RAM(reg1)) {
    EXPECT(RAM_IS_PAIR(reg1), "set-car!.0", "pair");

		RAM_SET_CAR_NO_TEST(reg1, reg2);
		reg1 = NIL;
		reg2 = NIL;
	}
  else {
...
BUILTIN_UNSPEC(set-cdr!, set_cdr_bang, 2, 11)
{
	if (IN_RAM(reg1)) {
    EXPECT(RAM_IS_PAIR(reg1), "set-cdr!.0", "pair");

		RAM_SET_CDR_NO_TEST(reg1, reg2);
		reg1 = NIL;
		reg2 = NIL;
	}
  else {
