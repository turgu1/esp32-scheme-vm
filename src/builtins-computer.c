...
loop:
    ...
    else if ((IN_RAM(o) && RAM_IS_STRING(o)) || (IN_ROM(o) && ROM_IS_STRING(o))) {
			printf("#<string>");
		}
...
BUILTIN_UNSPEC(print, print, 1, 38)
{
	print(reg1);
...
BUILTIN(clock, clock, 0, 39)
{
	reg1 = encode_int(read_clock());
}

BUILTIN(#%getchar-wait, getchar_wait, 2, 40)
{
	decode_2_int_args();
	a1 = read_clock() + a1;
...
BUILTIN_UNSPEC(#%putchar, putchar, 2, 41)
{
	decode_2_int_args ();
...
BUILTIN_UNSPEC(network-init, network_init, 0, 48)
{
...
BUILTIN_UNSPEC(network-cleanup, network_cleanup, 0, 49)
{
...
BUILTIN(receive-packet-to-u8vector, receive_packet_to_u8vector, 1, 50)
{
...
BUILTIN(send-packet-from-u8vector, send_packet_from_u8vector, 2, 51)
{
